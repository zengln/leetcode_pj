...
class Solution:
    def findInMountainArray(self, target: int, mountain_arr: 'MountainArray') -> int:
        # 找峰值
        nums = mountain_arr.length() - 1
        max_number_index = 0
        left, right = 0, nums
        while not max_number_index:
            mid = (left + right) // 2
            next = mountain_arr.get(mid + 1)
            temp = mountain_arr.get(mid)
            last = mountain_arr.get(mid - 1)
            if next - temp < 0 and last - temp < 0:
        ...
        # 判断峰值右边是否存在target
        left, right = max_number_index + 1, nums
        while left <= right:
            mid = (left + right) // 2
            temp = mountain_arr.get(mid)
            if temp == target:
